...
# --- Helper for parsing C-style struct logs ---
def parse_log_dict(s: str) -> Dict[str, Any]:
    """
    ...
    # But simple regex: match word followed by colon
    s = re.sub(r'(?<=[\{\s,])([a-zA-Z_]\w*)\s*:', r'"\1":', s)
    
    # 2. Quote hex values (simple heuristics for 0x...)
    # Matches ': 0x123' or ': -0x123', possibly followed by comma or brace
    # Ensure not already quoted
    # Allow optional negative sign just in case
    s = re.sub(r':\s*(-?0x[0-9a-fA-F]+)(?!")', r': "\1"', s)
    
    # 3. Quote simple string values (like 'full', 'partial') that are NOT quoted
    # Pattern: : followed by space, then alphabets, followed by comma or brace
    s = re.sub(r':\s*([a-zA-Z_]+)(?=\s*[,}])', r': "\1"', s)
    
    return parse_dict_safe(s)
        ...
        for msg_type, pattern in self.PATTERNS.items():
            ...
            if match:
                ...
                if msg_type in (models.SlabCreateData, models.SlabAllocObjData, models.SlabFreeSlabData):
                    ...
                    kv_pair = parse_log_dict(kv_pair)
                    ...
                    kv_pair = parse_log_dict(check_exists(match.named, 'as_obj', msg_type))
